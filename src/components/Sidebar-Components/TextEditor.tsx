...
export const TextEditor: React.FC<TextEditorProps> = ({ selectedCanvas }) => {
  ...
  const [backgroundColor, setBackgroundColor] = useState<string | TFiller>("#000000");
    ...
    <div className="p-4 bg-gray-100 rounded shadow-md">
      ...
      <div className="mb-2">
        ...
        <input type="range"
          ...
          onChange={(e) => {
            setFontSize(parseInt(e.target.value))
            updateTextProperty("fontSize", parseInt(e.target.value))
          }} min="1" max="120" step="1" id="text-font-size" />

      </div>
      <div className="mb-2">
        <label htmlFor="" className="block text-sm font-medium">Text Align</label>
        <div className="inline-flex rounded-lg shadow-2xs">
          <button onClick={() => {
            setTextAlign('left')
            updateTextProperty("textAlign", textAlign)
          }} type="button" className={`${textAlign === 'left' && 'bg-white'} py-2 px-3 inline-flex justify-center items-center gap-2 -ms-px first:rounded-s-lg first:ms-0 last:rounded-e-lg text-sm font-medium focus:z-10 border border-gray-200 bg-gray-300 text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 cursor-pointer`}>
            left
          </button>
          <button onClick={() => {
            setTextAlign('center')
            updateTextProperty("textAlign", 'center')
          }} type="button" className={`${textAlign === 'center' && 'bg-white'} py-2 px-3 inline-flex justify-center items-center gap-2 -ms-px first:rounded-s-lg first:ms-0 last:rounded-e-lg text-sm font-medium focus:z-10 border border-gray-200 bg-gray-300 text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 cursor-pointer`}>
            center
          </button>
          <button onClick={() => {
            setTextAlign('right')
            updateTextProperty("textAlign", 'right')
          }} type="button" className={`${textAlign === 'right' && 'bg-white'} py-2 px-3 inline-flex justify-center items-center gap-2 -ms-px first:rounded-s-lg first:ms-0 last:rounded-e-lg text-sm font-medium focus:z-10 border border-gray-200 bg-gray-300 text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 cursor-pointer`}>
            right
          </button>
        </div>
      </div>
      ...
      <div className="mb-2">
        ...
        <div className="flex items-center space-x-2">
          <input
            type="color"
            value={String(backgroundColor) || String(selectedText.backgroundColor)}
            onChange={(e) => {
            ...
            className="w-10 h-10 border rounded"
          />
          <span>{String(backgroundColor)}</span>
        </div>
      </div>
      <div className="mb-2">
        <label className="block text-sm font-medium">Text Stroke Color</label>
        <div className="flex items-center space-x-2">
          <input
            type="color"
      ...
      <div className="mb-2">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            updateTextProperty("lineHeight", parseFloat(e.target.value).toFixed(2))
          }}
          className="w-full  p-2 border rounded"
        />
        <input type="range"
          ...
          onChange={(e) => {
            setLineHeight(parseFloat(e.target.value))
            updateTextProperty("lineHeight", parseFloat(e.target.value).toFixed(2))
          }} min="1" max="10" step="0.1" id="text-font-size" />
