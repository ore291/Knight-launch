...
export default function Sidebar({ selectedCanvas }: SidebarProps) {
  ...
  switch (tab) {
    ...
    case 4:
      active = (
        <div>
          <ImageEditor selectedCanvas={selectedCanvas} />
        </div>
      );
      break;
    default:
  ...
  return (
    <aside className="w-74 no-scrollbar min-w-70 bg-gray-100 h-screen  border-r overflow-y-auto py-4 ">
      <div className="w-full">
        <ul className="flex  justify-around max-w-full bg-gray-300  py-2">
          <li
            className={`opacity-50 cursor-pointer px-3 py-2 text-xs whitespace-nowrap ${getActiveClass(
              1,
              "bg-[#eeeded] !opacity-100"
            )}`}
            onClick={() => toggleTab(1)}
          >
            Text
